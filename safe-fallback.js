(function () {
  ...
  function fotoPorTipoResiduo(tipo) {
    ...
    if (chave.indexOf("cinza") >= 0) {
      return "slide05.png";
    }
    if (chave.indexOf("lodo") >= 0) {
      return "slide06.png";
    }
    if (chave.indexOf("óleo") >= 0 || chave.indexOf("oleo") >= 0) {
      return "slide19.png";
    }
    if (chave.indexOf("químic") >= 0 || chave.indexOf("quimic") >= 0) {
      return "slide03.png";
    }
    if (chave.indexOf("recicl") >= 0) {
      return "slide05.png";
    }
    if (chave.indexOf("biomassa") >= 0) {
      return "slide11.png";
    }
    if (chave.indexOf("solo") >= 0) {
      return "slide17.png";
    }
    if (chave.indexOf("epi") >= 0 || chave.indexOf("absorvente") >= 0) {
      return "slide20.png";
    }
    return "slide01.png";
  }

  function renderMosaicoContainer(containerId, listaImagens) {
    var container = byId(containerId);
    if (!container || !Array.isArray(listaImagens) || listaImagens.length === 0) {
      return;
    }

    container.innerHTML = listaImagens.map(function (item) {
      var arquivo = item[0];
      var legenda = item[1];
      return [
        '<figure class="image-mosaic-item">',
        '<img src="assets/images/' + encodeURI(String(arquivo)) + '" alt="' + escapeHtml(legenda) + '" loading="lazy" />',
  ...
  function renderFluxoFallback() {
    var fluxo = byId("fluxo-list");
    if (!fluxo) {
      return;
    }
    ...
    fluxo.innerHTML = etapas.map(function (item, idx) {
      var foto = FOTOS_FLUXO_FALLBACK[idx] || FOTOS_FLUXO_FALLBACK[FOTOS_FLUXO_FALLBACK.length - 1];
      return [
        '<article class="timeline-step">',
        '<img class="item-photo" src="assets/images/' + encodeURI(String(foto[0])) + '" alt="' + escapeHtml(foto[1]) + '" loading="lazy" />',
  ...
  function renderPlanosFallback() {
    var planos = byId("planos-list");
    if (!planos) {
      return;
    }
    ...
    var planosBase = [
      {
        ...
        subtitulo: "Plano técnico de recuperação",
        itens: [
          "Contenção emergencial e controle de drenagem.",
          "Projeto executivo com topografia e ensaios de solo.",
          "Execução de obras de estabilização e revegetação."
        ]
      },
      {
        titulo: "Resposta a Vazamento de Óleo",
        subtitulo: "Plano preventivo e corretivo",
        itens: [
          "Interromper fonte, isolar área e conter derrame.",
          "Reparar equipamento e validar estanqueidade.",
          "Reforçar inspeções e evitar recorrência."
        ]
      }
    ];

    planos.innerHTML = planosBase.map(function (plano, idx) {
      var foto = FOTOS_PLANOS_FALLBACK[idx] || FOTOS_PLANOS_FALLBACK[FOTOS_PLANOS_FALLBACK.length - 1];
      var itens = plano.itens.map(function (item) {
  ...
  function renderFotosFallback() {
    renderMosaicoContainer("dashboard-images", IMAGENS_SECOES_FALLBACK.dashboard);
    renderMosaicoContainer("residuos-images", IMAGENS_SECOES_FALLBACK.residuos);
    renderMosaicoContainer("tratamento-images", IMAGENS_SECOES_FALLBACK.tratamento);
    renderMosaicoContainer("planos-images", IMAGENS_SECOES_FALLBACK.planos);
    renderMosaicoContainer("relatorio-images", IMAGENS_SECOES_FALLBACK.relatorio);
    renderFluxoFallback();
    renderPlanosFallback();
  }
  ...
  function bootFallback() {
    ...
    var filterToggle = byId("residue-filter-toggle");
    var filterPanel = byId("residue-filter-panel");
    if (filterToggle && filterPanel) {
      filterToggle.addEventListener("click", function () {
        var abrir = filterPanel.hidden;
        filterPanel.hidden = !abrir;
        filterToggle.classList.toggle("is-open", abrir);
        filterToggle.setAttribute("aria-expanded", abrir ? "true" : "false");
        filterToggle.textContent = abrir ? "Ocultar filtros" : "Filtros";
      });
    }
